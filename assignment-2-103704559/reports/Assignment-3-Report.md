...
This assignment extends the mysimbdp platform from Assignment 2 with stream analytics capabilities. The same infrastructure (Kafka, Cassandra, Docker) is reused. A PySpark Structured Streaming job is added that consumes raw sensor readings in real time, computes sliding-window aggregations per country, detects PM2.5/PM10 threshold breaches, writes analytics results to Cassandra, and publishes alerts back to the tenant via a dedicated Kafka topic.
...
**Architecture diagram:** `[ADD ARCHITECTURE DIAGRAM HERE — draw the flow: sensor.community API → tenanta-producer → Kafka → spark-streaming → Cassandra + tenantA.alerts → tenant-alert-consumer]`
...
**Dataset:** sensor.community open air quality API (`https://data.sensor.community/static/v2/data.json`). This API returns a rolling snapshot of readings from ~15,000 citizen-owned air quality sensors worldwide. Each record contains a sensor identifier, geographic location (lat/lon/country), and particulate matter measurements (PM2.5 as P2, PM10 as P1) from sensors like the SDS011.

**Why suitable for streaming analytics:** The API updates every 5 minutes with new sensor readings from thousands of globally distributed sensors. This is a continuous, high-volume, geographically distributed data source — exactly the kind of scenario where streaming analytics adds value over batch. Air quality is time-sensitive: a PM2.5 spike that causes a public health alert needs to be detected within minutes, not hours.

**Scenario:** TenantA is a public health monitoring service. They stream raw sensor readings into the platform in near real-time. The `streamanalyticsapp` computes rolling average PM2.5 and PM10 concentrations per country over sliding 5-minute windows. When the average PM2.5 in a country exceeds 15 μg/m³ (WHO 24h guideline) or PM10 exceeds 45 μg/m³, an alert is fired back to the tenant immediately.

**streamanalyticsapp functionality:**

- Consumes from Kafka topic `tenantA.bronze.raw`
- Groups records by country and 5-minute sliding window (slides every 1 minute)
...
**Keyed stream:** The stream is keyed by `country`. In the Kafka producer, messages are partitioned by `sensor_id` (ensuring all readings from the same sensor go to the same partition, preserving per-sensor ordering). In PySpark, the windowed aggregation groups by `(window, country)`, making it effectively keyed on country for analytics purposes.

Keying by country is appropriate because:
...
**Message delivery guarantees:** At-least-once delivery is appropriate and sufficient for this scenario. The producer uses `enable.idempotence=True` and `acks=all` which provides exactly-once delivery at the producer level within a single Kafka session. On the consumer (Spark) side, `foreachBatch` with checkpointing provides at-least-once semantics — if a batch fails and retries, some window results may be recomputed and re-written to Cassandra, but since Cassandra writes use the same primary key `(country, window_start)` the result is idempotent (same row gets overwritten with the same data).

Exactly-once end-to-end would require Kafka transactions and two-phase commit with Cassandra, which adds significant complexity. For a public health monitoring scenario, at-least-once is acceptable: a duplicate alert is far less harmful than a missed alert.
...
If a record has no timestamp, the fallback is to use `kafka_ts` (the Kafka broker timestamp, which is processing time) as a best-effort substitute.

**Window type and parameters:**

- **Sliding window**: 5-minute duration, sliding every 1 minute
- This means each record contributes to 5 overlapping windows simultaneously
- A 5-minute window is appropriate because the source API updates every 5 minutes — each window captures approximately one full API refresh cycle of data
- Sliding by 1 minute gives finer temporal resolution for detecting rapidly developing pollution events
- Alternative tested: 1-minute tumbling window (WINDOW_DURATION=1 minute, WINDOW_SLIDE=30 seconds) — produces more rows with lower record counts per window

Window function used: `groupBy(window(event_time, "5 minutes", "1 minute"), country)` with aggregation functions `avg`, `max`, `count`.
...
[sensor.community API]
        │  HTTP GET every 0.5s
        ▼
[tenanta-producer container]
  tenantA_producer.py
  — fetches random sensor reading
  — builds flat JSON event
  — publishes to Kafka (key=sensor_id)
        │
        ▼
[Kafka broker — mysimbdp messaging system]
  topic: tenantA.bronze.raw  (6 partitions)
  topic: tenantA.alerts      (6 partitions)
        │                          ▲
        │ consume                  │ publish alerts
        ▼                          │
[spark-streaming container — mysimbdp streaming engine]
  streamanalyticsapp.py (PySpark Structured Streaming)
  — readStream from tenantA.bronze.raw
  — watermark 2 min, sliding window 5min/1min
  — groupBy(window, country) → avg/max/count
  — foreachBatch → write to Cassandra + publish alerts
        │
        ▼
[Cassandra — mysimbdp-coredms]
  tenanta_analytics.window_results

[tenant-alert-consumer container]
  alert_consumer.py
  — subscribes to tenantA.alerts
  — prints 🚨 ALERT RECEIVED in real-time
```
